...
-(void) didMoveToView:(SKView *)view{
    ...
    _img.image = [UIImage imageNamed:@"Tennis.png"];
    _img.hidden = NO;
    ...
    question.text = @"What sport is this ball used in?";
    question.name = @"questionNode";
    question.fontSize = 30;
    question.position = CGPointMake(CGRectGetMidX(self.frame),700);
    //Create bball, soccer, and tennis nodes
    SKLabelNode *tennisNode = [SKLabelNode labelNodeWithFontNamed:@"Chalkduster"];
    tennisNode.text = @"Tennis";
    tennisNode.name = @"tennisNode";
    tennisNode.fontSize = 18;
    tennisNode.position = CGPointMake(300, 50);
    SKLabelNode *basketballNode = [SKLabelNode labelNodeWithFontNamed:@"Chalkduster"];
    basketballNode.text = @"Basketball";
    basketballNode.name = @"basketballNode";
    basketballNode.fontSize = 18;
    basketballNode.position = CGPointMake(CGRectGetMidX(self.frame), 50);
    SKLabelNode *soccerNode = [SKLabelNode labelNodeWithFontNamed:@"Chalkduster"];
    soccerNode.text = @"Soccer";
    soccerNode.name = @"soccerNode";
    soccerNode.fontSize = 18;
    soccerNode.position = CGPointMake(700, 50);
    ...
    [self addChild:tennisNode];
    [self addChild:basketballNode];
    [self addChild:soccerNode];
    
}

-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event { //if happy Node pressed (matches scene, move onto next scene
    //Nodes
    SKNode *tennisNode = [self childNodeWithName:@"tennisNode"];
    SKNode *basketballNode = [self childNodeWithName:@"basketballNode"];
    SKNode *soccerNode = [self childNodeWithName:@"soccerNode"];
    ...
    for(UITouch *touch in touches){
        CGPoint tennisLocation = [touch locationInNode:self];
        CGPoint basketballLocation = [touch locationInNode:self];
        CGPoint soccerLocation = [touch locationInNode:self];
        CGPoint restartLocation = [touch locationInNode:self];
        CGPoint exitLocation = [touch locationInNode:self];
        
        if([tennisNode containsPoint:tennisLocation]){
            //create actions
            ...
            [UIImageView animateWithDuration:0.5f
                                  ...
                                  completion:^(BOOL finished) {
                                      ...
                                      [tennisNode runAction:moveSequence completion:^{
                                          switch (randNum) {
                                              case 1:
        ...
        if ([basketballNode containsPoint:basketballLocation]){ //wrong location
            //create actions
            SKAction *shrink = [SKAction scaleTo:0.5 duration:0.05];
            [basketballNode runAction:shrink];
        }
        if ([soccerNode containsPoint:soccerLocation]){ //wrong location
            SKAction *shrink = [SKAction scaleTo:0.5 duration:0.05];
            //run action
            [soccerNode runAction:shrink];
        }
