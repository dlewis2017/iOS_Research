...
-(void) didMoveToView:(SKView *)view{
    ...
    _img.image = [UIImage imageNamed:@"CircleCookie.png"];
    _img.hidden = NO;
    ...
    question.text = @"What shape is this cookie?";
    ...
    circleNode.position = CGPointMake(300, 50);
    ...
    triangleNode.position = CGPointMake(CGRectGetMidX(self.frame), 50);
    ...
    squareNode.position = CGPointMake(700, 50);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event { //if happy Node pressed (matches scene, move onto next scene
    ...
    for(UITouch *touch in touches){
        ...
        if([circleNode containsPoint:circleLocation]){
            ...
            [circleNode runAction:moveSequence completion:^{
                switch (randNum) {
                    case 1:
