...
-(void) didMoveToView:(SKView *)view{
    ...
    _img.image = [UIImage imageNamed:@"TrianglePizza.png"];
    ...
    question.text = @"What shape is the pizza slice?";
    ...
    triangleNode.position = CGPointMake(300, 50);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event { //if happy Node pressed (matches scene, move onto next scene
    ...
    for(UITouch *touch in touches){
        ...
        if([triangleNode containsPoint:triangleLocation]){
            ...
            [triangleNode runAction:moveSequence completion:^{
                switch (randNum) {
                    case 1:
