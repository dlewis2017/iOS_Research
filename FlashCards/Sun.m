...
-(void) didMoveToView:(SKView *)view{
    ...
    _img.image = [UIImage imageNamed:@"Sun.png"];
    _img.hidden = NO;
    ...
    rainNode.position = CGPointMake(CGRectGetMidX(self.frame), 50);
    ...
    lightningNode.position = CGPointMake(700, 50);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event { //if happy Node pressed (matches scene, move onto next scene
    ...
    for(UITouch *touch in touches){
        ...
        if([sunNode containsPoint:sunLocation]){
            //create actions
            ...
            [UIImageView animateWithDuration:0.5f
                                  ...
                                  completion:^(BOOL finished) {
                                      ...
                                      [sunNode runAction:moveSequence completion:^{
                                          switch (randNum) {
                                              case 1:
        ...
        if ([lightningNode containsPoint:lightningLocation]){ //wrong location
            SKAction *shrink = [SKAction scaleTo:0.5 duration:0.05];
            //run action
            [lightningNode runAction:shrink];
        }
