...
//  FlashCardGame
//
//  Created by C Poellabauer on 3/16/15.
...
-(void) didMoveToView:(SKView *)view{
    ...
    _img.image = [UIImage imageNamed:@"AngryFace3.png"];
    _img.hidden = NO;
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event { //if happy Node pressed (matches scene, move onto next scene
    ...
    for(UITouch *touch in touches){
        ...
        if([angryNode containsPoint:angryLocation]){
            //create actions
            ...
            [angryNode runAction:moveSequence completion:^{
                switch (randNum) {
                    case 1:
        ...
        if ([happyNode containsPoint:happyLocation]){
            ...
            [happyNode runAction:shrink];
        }
        if ([sadNode containsPoint:sadLocation]){
            //create actions
            //SKAction *red = [SKAction colorizeWithColor:[SKColor redColor] colorBlendFactor:1.0 duration:.5];
            SKAction *shrink = [SKAction scaleTo:0.5 duration:0.05];
            //run sequence
            //SKAction *moveSequence3 = [SKAction sequence:@[red,shrink]];
